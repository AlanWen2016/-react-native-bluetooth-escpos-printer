...
RCT_EXPORT_METHOD(printInHex:(NSString *) str
                  andResolver:(RCTPromiseResolveBlock) resolve
                  rejecter:(RCTPromiseRejectBlock) reject)
{
    NSMutableData *toPrint = [[NSMutableData alloc] init];
    Byte byte[4] = {27, 64, 27, 51};
    // byte = [self stringToByte:str];
    // NSData *bytes = [[NSData alloc] initWithBytes:[self stringToByte:str] length:sizeof([self stringToByte:str])];
    // Byte bytes[16] = {0};
    // bytes = [self stringToByte:str];
    // [toPrint appendBytes: bytes length: sizeof(bytes)];
    // [toPrint appendData: bytes];
    // NSData *bytes = [str dataUsingEncoding:[self toNSEncoding:encoding]];
    [toPrint appendBytes: byte length: 4];
    // [toPrint appendData: [self stringToByte:str]];
    NSLog(toPrint);
    pendingReject = reject;
    pendingResolve = resolve;
...
//     }
//     NSLog(@"bytes size for int : %d \n", sizeof(bytes));
//     NSData *nsBytes = [[NSData alloc] initWithBytes:bytes length:sizeof(bytes)];
//     return nsBytes;
// }
...
- (NSData*)stringToByte:(NSString*)string {
    ...
    if (len % 2 == 1) {
        len++;
        halfLen = len / 2;
        string = [firstStr stringByAppendingString:string];
    } else {
        halfLen = len / 2;
    }
    Byte bytes[halfLen];
    ...
    for (int i = 0; i < len - 2; i += 2) {
        NSString *targetStr = [string substringWithRange:NSMakeRange(i, 2)];
        // char *ValueasCString = (char *)[targetStr UTF8String];
        bytes[j] = [self numberWithHexString: targetStr];
        j++;
    }
    NSData *nsBytes = [[NSData alloc] initWithBytes:bytes length:sizeof(bytes)];
    return nsBytes;
