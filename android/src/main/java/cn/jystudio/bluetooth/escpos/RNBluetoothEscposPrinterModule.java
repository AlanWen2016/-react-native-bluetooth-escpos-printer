...
//                String text = new String(toPrint, Charset.forName(encoding));
    ...
    // 根据16进制编码进行打印
    @ReactMethod
    public void printInHex(String inHex,final Promise promise){
        if(sendDataByte(hexToByteArray(inHex))){
            promise.resolve(null);
        }else{
            promise.reject("COMMAND_NOT_SEND");
        }
    ...
    @ReactMethod
    public void printInBytes(ReadableArray jsBytes,final Promise promise){
        int size = jsBytes.size();
        byte bytes[] = new byte[size];
        for (int i = 0; i < size; i ++) {
            // bytes.add(jsBytes.get)
        }
        if(sendDataByte(bytes)){
            promise.resolve(null);
        }else{
